...
長さを見るために、**2026-08-01 の時点でコレクションに入っている 57 本**を材料にした。
ユーザーが実際に取り込んだものがそのまま全部で、この文書のために選んだり除いたりはしていない。
分野は計算機グラフィックスに偏っており、題に `Real-Time` や `Neural` のような複合語と修飾が多い。

この 57 本の slug は中央 46 文字・最長 57 文字で、`@wang2018-analytic-spherical-harmonic-coefficients` は 49 文字である。
日本語の 1 文がおよそ 30 文字から 50 文字なので、参照 1 つで 1 文ぶんの場所を取る。
別の分野では語の長さの分布が違うが、姓と年に続けて題の語を並べる以上、参照が 1 文ぶんに達すること自体は変わらない。
...
実線の矢印は「左のものから右のものを作る」、点線の矢印は「ツールチップを押したときに移る先」を表す。
...
`Wang 2026` はこのコレクションに 2 本(`wang2026-himat` と `wang2026-restir-g-pt`)あるので字が付き、`Zhang 2026` は 1 本なので付かない。
...
- 正式な slug(`@wang2026-himat`)。

**ツールチップの中を押すと、その論文の詳細へ移る。**
題と slug はどちらも同じ論文を指すので、押せる場所を分けない。
...
1 本あたり数百バイトで、上記の 57 本なら 10 KB 台である。
...
ツールチップを押すと、左の欄を論文の表示に切り替え、その論文の詳細を開く。
...
- **短縮された名前をコピーしても slug にならない。** 本文を選んで別の場所へ貼ると `@Wang 2026a` が入る。slug が要るときは、ツールチップで読むか、markdown のファイルを読むことになる。
