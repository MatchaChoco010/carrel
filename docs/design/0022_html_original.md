...
- **数式が構造を持ったまま入っている。** arXiv が出す HTML(LaTeXML 由来)は数式を MathML で書き、その中に元の LaTeX を `alttext` として持っている。PDF の紙面から読み直すのとは違い、著者が書いた式そのものが手に入る。
...
arXiv の HTML でしか読めない論文を取り込む。

**取得**が `https://arxiv.org/html/2604.12345v1` を保存する。
...
1. `<article>`、`<main>`、`role="main"` の要素があればそれを採る。arXiv の HTML も多くの出版社のページもこのいずれかを持つ。
2. 無ければ、文章の量が最も多い要素を選ぶ。見出しと段落の文字数を数え、案内の欄のような短い塊を外す。

選んだ塊からは、案内(`nav`)、頭と足(`header`、`footer`)、傍注(`aside`)、`script`、`style`、フォーム、ボタンを落とす。

この選び方を外したときは、本文が短すぎる形で残る。
**取り出した本文が短すぎるときは変換を失敗させる**(段階の失敗として一覧に出る)。
黙って中身の薄い論文を登録すると、後の翻訳と索引まで薄いまま進んでしまう。
...
- **HTML を PDF に変換してから、いまの変換器で読む。** Marker は HTML を weasyprint で PDF にしてから読む経路を持っており、これを使えば照合もそのまま通る。weasyprint という native の依存(pango と cairo)が増える。加えて、arXiv の HTML が持っている LaTeX を捨てて、紙面の画像から数式を読み直すことになる。取り出せる情報を捨てる方向なので採らない。
