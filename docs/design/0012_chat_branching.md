...
carrel のチャットは Codex のスレッドの上で動き(0003)、エージェントが何を覚えているかはスレッドが決める。
...
また carrel はユーザーの発言をターンの開始前に記録へ書く一方、識別子が分かるのは `turn/started` を受けた後である。
...
- **人が読む markdown の見出しに 36 文字の識別子が並ぶ。** 発言の区切りを目で追うときの雑音になる。carrel の画面では表示しない。
- **読み込み直しを経た会話は、中間状態を引き継いだ分岐ができない。** その会話のどの地点から分岐しても、記録から立て直す経路になる。Codex 側にその状態が残っていないため、carrel 側で補う手段はない。
...
- **識別子を HTML コメントとして発言の直後に置く。**
  ...
  一方で、carrel が無い環境で markdown を開いたときに、その値が何なのかを読み取れない。
