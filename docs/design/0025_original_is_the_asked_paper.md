...
- **題の語が原本の先頭に現れるかで、機械的に判じる。** Codex を使わないので速く、枠も使わない。コレクションの 85 本で測ると、正しい組は題の語がすべて先頭 2 ページに現れ(1.00)、混入した企業文書は 1 語も現れなかった(0.00)ので、無関係な文書は弾ける。その論文に付随する文書・引用した特許・同じ著者の別の論文は、どれも題と著者をそのまま含むので弾けない。文字層を持たない走査した紙面も判じられない。防ぎたいのは「別の文書が入ること」なので、これでは足りない。
