...
エージェント側の事情もある。
Codex の会話スレッドは作業ディレクトリを `$CARREL_DATA` に置いて読み取り専用で動くため、論文の markdown をファイルとして読むことはできる(0003)。
しかし埋め込みによる検索は carrel のサーバーの中にしかないため、エージェントに使わせるには何らかの口が要る。
...
エージェントへの公開は、carrel のサーバー自身が持つ MCP の口で行う。
会話スレッドだけがこれを接続する。
...
carrel は日本語のその語句をそのまま埋め込みにして、英語の本文と和訳の両方のベクトルを引く。
...
carrel のサーバーは、自身の HTTP のサーバー上に MCP の口を持つ。
...
これはエージェントが元から持っている能力であり、carrel が肩代わりする理由がない。
...
- **エージェントが検索を使うかはモデルの判断による。** 道具を与えても、エージェントが使わずに自分の知識だけで答えることがある。`$CARREL_DATA/AGENTS.md` で使い方を伝えるが(0002)、使用を強制はできない。
...
MCP の口は carrel の HTTP サーバー上にあり、サーバーと同じ範囲にしか公開されない(0001)。
公開する操作は読み取りだけなので、この口を通じてコレクションが変更されることはない。

エージェントに渡すのは検索の結果とファイルの場所であり、エージェントはサンドボックスの読み取り権限の範囲でファイルを読む(0003)。
サンドボックスの範囲は `$CARREL_DATA` に限られるため、この口がコレクションの外への到達手段になることはない。
