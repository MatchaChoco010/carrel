...
- **`references` を `register` の後に置いたまま、参考文献がまだ無い論文では本文の節をそのまま出す。** 登録が 1〜2 分早く、参考文献の失敗が論文の使用を妨げない。参考文献を持たない論文が正常な状態として存在し続け、本文の中の導線がある論文と無い論文に分かれるので採らない。
...
段階の並べ替えで、取り込み 1 本の登録が 1〜2 分遅れる。
変換と照合と翻訳で 10〜40 分かかる行程なので、論文が読めるようになる時刻はその範囲で動く。
