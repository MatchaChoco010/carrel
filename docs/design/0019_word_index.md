...
- **純粋な JavaScript で動く。** ビルドの手順や native の依存が要らず、`pnpm install` だけで揃う。carrel は Node の標準の SQLite を使うなど、native の追加を避ける作りにしてある。
...
- **native や wasm の解析器(lindera、vibrato、MeCab)を使う。** 解析が速く、新しい辞書(UniDic など)も選べる。ビルドの手順か配布物の取り込みが要り、carrel が避けてきた native の依存が増える。速度は取り込みの待ちに埋もれる程度なので、まず純粋な JavaScript で済ませる。
