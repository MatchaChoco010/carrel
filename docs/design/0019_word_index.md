...
- **辞書を持つ形態素解析器を入れない。** 分野の語彙に強くなるが、辞書(数十 MB)と依存が増える。まず標準で使える分割で測り、それでも取りこぼすなら入れ替えを検討する。
...
索引を作るときと問い合わせるときに同じ分割を通すことで、両側の語の切れ目が揃う。

分かち書きには Node に入っている ICU の語分割(`Intl.Segmenter`)を使い、新しい依存を足さない。
表示する本文と抜粋は元の文字のままで、分かち書きした語列は索引にだけ入る。
会話の検索も同じ索引の形を使っているので、同じように変える。

実測では、2 文字の語(照明・光源・影)が引けるようになり、`art` は語としての `art` にだけ当たる。
索引は 11.68 MB から 5.10 MB に減る。
...
Node に入っている ICU の語分割(`Intl.Segmenter`)を使う。
日本語の語分割を辞書つきで行い、英語は空白と記号で区切られる。

**辞書を持つ解析器を足さない。**
ICU の分割は分野の語彙に弱く、複合語(球面調和関数)は要素に割れる。
それでも取りこぼしにならないのは、問い合わせ側も同じ分割を通すためである。
「球面調和関数」を打てば同じ並びに割れ、語の並びとして一致する。
...
- **切り方を外すと当たらない。** 3 文字の並びの索引は語の切れ目を知らないぶん取りこぼしが無かった。語の単位にすると、索引側と問い合わせ側で切れ目が食い違った語は当たらなくなる。両側で同じ分割を通すので、食い違うのは前後の文脈で分割が変わる場合に限られる。
- **ICU の語彙は分野語に弱い。** 新しい術語や固有名詞は要素に割れる。並びとして照会するので当たるが、要素が一般的な語だと関係の薄いチャンクも当たる。
...
- **辞書を持つ形態素解析器(kuromoji など)を入れる。** 分野の語彙に強く、複合語を意図どおりに切れる。辞書が数十 MB あり、依存も増える。標準の分割で足りるかを先に測るべきなので採らない。取りこぼしが問題になれば入れ替える。
...
メイン案を選ぶ基準は、**打った語がそこに書いてあるときに確実に当たること**と、**依存を増やさないこと**である。
語の単位にすることで前者を満たし、Node に入っている分割を使うことで後者を満たす。
...
- **分かち書き**: 全チャンクで 0.4 秒。取り込みでは 1 本ぶんが数十ミリ秒になり、変換や翻訳の待ちに埋もれる。問い合わせのたびに走るのは語句 1 つぶんで、測れる量ではない。
- **索引の大きさ**: 3 文字の並びで 11.68 MB、語の単位で 5.10 MB。半分以下になる。
