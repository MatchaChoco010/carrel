...
carrel のチャットは Codex のスレッドの上で動く(0003)。
Codex はスレッドの内容を自分のセッション保存領域にファイルとして持ち、そのファイルを再生することで会話を再開する。
この保存領域は Codex の認証情報と同じ場所にあるため、`$CARREL_DATA` には置かない(0001)。
...
- **削除した会話を復元する仕組みを持たない。** 削除は取り消せない。誤操作からの保護は確認の手順で行い(後述)、削除後の復旧はバックアップからの取り出しに委ねる。ごみ箱のような中間状態を設けると、アーカイブとの違いがユーザーにとって分かりにくくなる。
- **Codex 側の状態を `$CARREL_DATA` へ複製しない。** スレッドの状態ファイルだけをバックアップ対象へ複製する設計であり、「代替案」で詳しく述べる。Codex の内部形式への依存を受け入れる判断をしない限り、恒久的に採らない。
- **会話をまたいだ文脈の共有を行わない。** ある会話でエージェントが得た理解を別の会話へ引き継ぐ仕組みは持たない。会話は独立した単位とし、必要な文脈は都度渡す。会話をまたぐ知識の蓄積は、論文コレクションそのものが担う。

## 概要

carrel のチャットは `chats/` の markdown ファイルを正とし、Codex のスレッドはそれに付随する実行状態として扱う。
...
会話を片付ける手段は 2 つある。
**アーカイブ**は「この議論はもう活発ではない」という整理で、内容は残る。carrel 側の状態として frontmatter に持ち、解除するまで読み取り専用になる。
**削除**は誤って作った会話を消すための操作で、内容ごと無くなる。
普段使うのはアーカイブで、削除は押し間違いなどで中身の無い会話ができたときに使う。
そのため削除は、一覧の項目のメニューから選び、確認を経てからでないと実行されない。
...
以下では、markdown と Codex スレッドの関係、再開の判定と読み込み直し、分岐、アーカイブ、削除、タイトルと要約の生成、`@slug` の扱い、チャットの検索を順に述べる。
...
会話を開くとき、carrel は `codex_thread_id` に対応するスレッドが Codex 側に存在するかを確認する。
存在すればそのまま続きを話せる。
...
アーカイブは carrel 側の状態として frontmatter に持つ。
...
削除は、会話の markdown ファイルを消し、索引からその会話を取り除き、対応する Codex のスレッドも破棄する操作である。
アーカイブと違って内容は残らない。

**削除に至る経路は 1 つだけにする。**
一覧の項目のメニューから削除を選び、確認を経てから実行する。
一覧やチャット欄に、押すだけで消えるボタンは置かない。
論文の削除と同じ扱いである(0002)。

この保護が要るのは、削除とアーカイブが隣り合う操作だからである。
どちらも「この会話を片付ける」という同じ意図から選ばれるのに、片方は取り消せて片方は取り消せない。
操作の重さを手順の長さで差し付けないと、アーカイブのつもりで削除する事故が起きる。

削除の対象になるのは、押し間違いなどで作られた中身の無い会話や、始めてすぐに方向が違うと気づいた会話である。
議論として成立した会話を片付ける手段はアーカイブであり、そちらが普段使う操作になる。

Codex のスレッドも一緒に破棄するのは、会話の記録が無くなればそのスレッドを使う経路が無くなるからである。
残しておいてもセッションの保存領域を占めるだけになる。

削除した会話を `forked_from` で指していた分岐先の会話は、由来を辿れなくなる。
このとき分岐先の会話は書き換えない。
分岐先には分岐点までの発言が書き写されているため(前述)、由来が辿れなくても本文としては完結している。
...
このとき carrel は、チャットの markdown を書き換えない。
...
- **削除は取り消せない。** 確認を挟むだけで、実行された後に戻す手段はアプリケーション内に無い。NAS のバックアップから取り出すことになる。
- **削除した会話を指す `forked_from` は解決できなくなる。** 分岐先の会話は読めるが、どの会話から分かれたのかを辿れなくなる。
...
- **Codex の保存領域を `$CARREL_DATA` の下に置く。**
...
- **Codex の保存領域から、スレッドの内容だけを `$CARREL_DATA` へ複製する。**
...
- **削除を持たず、アーカイブだけで片付ける。**
  ...
  会話を消す手段を用意せず、不要なものはアーカイブで視界から外す設計である。
  取り消せない操作が無くなるため、誤操作で失われるものが原理的に存在しない。
  一方で、押し間違いで作られた中身の無い会話が一覧に残り続ける。
  アーカイブは「議論としては終わった」という意味を持つ整理なので、そこに空の会話が混ざるとアーカイブ自体の意味が薄れる。
  ファイルを直接消せば片付くが、そのためだけに端末を開くのは道具として不親切である。

- **削除した会話をごみ箱へ移し、後から戻せるようにする。**

  削除を即時のファイル削除ではなく、復元できる中間状態への移動とする設計である。
  誤って削除しても取り返せる。
  一方で、会話の状態に「アーカイブ済み」と「ごみ箱」という似た 2 つの片付けられた状態が並ぶ。
  どちらもファイルは残っていて一覧から消えているため、ユーザーは 2 つの違いを覚えることになる。
  確認の手順で誤操作を防げるなら、状態を 1 つ増やす必要はない。

- **アーカイブに Codex の同名の操作を使う。**

  carrel のアーカイブ操作を Codex 側のスレッドのアーカイブに対応させる設計である。
  状態を 1 箇所で持てる。
...
この設計が新たに扱う機微なデータはない。
会話の内容は 0002 の定めるとおり `$CARREL_DATA` に保存され、Codex へ送られる内容の扱いは 0003 に従う。
