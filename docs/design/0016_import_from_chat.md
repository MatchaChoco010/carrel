...
carrel は自分が立てた MCP のサーバーからの要求を自動で受け入れる(0003 の承認の扱い)。
...
carrel は自分の MCP のサーバーからの要求を自動で受け入れる(0003)。
...
- **道具を足さず、会話の中で carrel が「取り込んで」を読み取る。** MCP を広げずに済む。応答の文面から意図を読み取る処理を carrel 側に持つことになり、言い回しの違いで動いたり動かなかったりする。エージェントに道具として渡すほうが確実なので採らない。
...
MCP の口はループバックで待ち受け、carrel が立てたスレッドからのみ繋がる(0005、0007)。
