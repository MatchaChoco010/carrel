...
ブラウザで開けば PDF は保存できるので、それを落とし、carrel の取り込みの欄の隣にあるボタンから、そのファイルを選ぶ。
...
コレクション(`$CARREL_DATA`)に置くのは論文と会話だけであり(0002)、slug の決まっていない原本はどちらでもない。
...
- **取り込みを取り消すと原本も消える。** 消した後にもう一度入れるには、同じ PDF を上げ直す。手元の PDF は carrel の外にあるので、ユーザーの手元に残っていることが前提になる。
