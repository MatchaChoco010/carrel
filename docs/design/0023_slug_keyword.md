...
- **規則ごと解決に渡して語幹を作らせ、規則で検証して落ちたら機械の結果に戻す。** どこで切れば題を思い出せるかは長さの数字では決まらないので、その一点だけを判断に任せられる。解決の問い合わせに欄を足すだけなので、やりとりは増えない。57 本で試すと、規則を全部渡しても 3 本で語数の上限を破り、コロンの略称の規則は 9 本で無視した。落ちたぶんは機械の結果に戻ることになり、規則を渡す手間に見合わない。切り所そのものも規則より短い側に寄り(語幹の長さが中央で 35 文字から 28 文字へ)、当たる語が減った。
...
題の語だけを使い、当たりに寄与しない語を落とし、本文に並べて邪魔にならない長さに収める。
