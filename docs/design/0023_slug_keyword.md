...
| `Clustering on the Unit Hypersphere using von Mises-Fisher Distributions` | `von-mises-fisher` | 固有名詞として取り込みが指した(下記) |
...
飛ばすと `mises-fisher` になり、題の記憶と食い違う。
...
**解決が語幹を提案するとき、飛ばす語をあえて含めたことを明示できる**ようにする。
明示されたときだけ、その語を落とさずに使う。

明示が無ければ規則どおりに飛ばす。
規則を捨てて解決に任せきりにしないのは、同じ問い合わせでも返る語が揺れるためである。
明示があっても、次の検査は変わらず効かせる。

- その語がすべて題に出てくること(頭字語の発明を弾く)。
- 5 語・32 文字までに収まること。

つまり、規則が既定で、解決が明示したときだけ「飛ばす語を残す」1 点を上書きする。

### 提案された語の扱い

取り込みの解決は、語幹を提案できる(0004)。
提案として受け取るのは、論文が題の中で与えている略称と、題の中の固有名詞である。

提案された語は、**その語がすべて題に出てくるときだけ**使う。
出てこない語は、頭文字をつないで作られたものとみなして捨て、題から作り直す。

提案には、飛ばす語をあえて含めたかどうかを添えられる(前節)。
添えられていないときは、提案された語にも規則の飛ばしを当てる。
