...
翻訳、要約、変換結果とページ画像の照合、そして論文についての議論という性質の違う仕事が、Codex という 1 つの資源を共有して走る。
...
carrel が AI に投げる仕事は、消費する量が桁違いに異なる。
arXiv の abstract の和訳は数百トークンで済む。
論文本文の全訳は入力と出力を合わせて数万トークンになる。
照合は論文の全ページの画像を送り、ページごとに本文を返させるため、1 本で入力が 3 万トークン、出力が 1 万トークンの規模になる。
...
- **枯渇時にローカルモデルへ退避しない。** 翻訳や要約を枠の枯渇時だけ手元の Ollama で処理する設計であり、「代替案」で詳しく述べる。翻訳の質が実行時刻で変わることを受け入れる判断をしない限り、恒久的に採らない。
...
- **ジョブ** — carrel が非同期に実行する仕事の単位。AI を使うものと GPU を使うものがある。
- **会話スレッド / 作業スレッド** — Codex のスレッドの 2 つの使い方。後述する。

## 概要

carrel は Codex のスレッドを 2 通りに使い分ける。
**会話スレッド**はユーザーとの議論そのもので、永続し、論文ディレクトリを読め、コレクションを検索する道具を持ち、モデルと reasoning effort をユーザーが選ぶ。
**作業スレッド**は翻訳や照合といった 1 回きりの処理で、使い捨てとして開始し、指示と道具を最小限にして、応答の形を JSON Schema で固定する。
...
変換が終わった論文から照合と翻訳が始まり、これらは Codex の枠を使う。
...
作業ディレクトリを `$CARREL_DATA` に置き、サンドボックスは読み取り専用にする。
コレクションを検索する MCP の口を接続し(0005)、`$CARREL_DATA/AGENTS.md` によってコレクションの構造を伝える。
...
carrel を通さずに端末から Codex を使った消費も、この数字に含まれる。
そのためこの表示は「carrel がどれだけ使ったか」ではなく「アカウントの残りがどれだけか」を意味する。

### 承認要求の扱い

Codex はサンドボックスの外で何かを実行しようとするとき、クライアントへ承認を求める。
carrel はこの要求を自動的に拒否し、拒否したことを会話に表示する。
ユーザーに承認を求める画面は出さない。
...
- **残枠の表示はアカウント全体の値である。** carrel 以外の Codex の利用も同じ枠を消費するため、表示された残量は carrel が使える量とは限らない。
...
- **照合と本文の全訳は、1 本あたりの消費が abstract の和訳の数十倍になる。** 取り込み 1 本で会話 2〜3 ターンぶんを消費するため、取り込みを連続して行うと、フィードを何日ぶん眺めるよりも早く枠が減る。
...
| 照合 | 3 万トークン規模(全ページの画像と本文) | 1 万トークン規模 | 取り込み 1 本ごと |
