...
論文をまとめて取り込んだとき、先に取り込みを頼んだ 1 本が先に読めるようになる。
...
- **待ち時間の長さで順序を動かさない。**
  ...
  後から取り込みを頼んだ論文が延々と待たされる状態を防げる。
  しかし取り込みを始めるのは人であり、取り込みを頼み続けない限り列はいつか空く。
  順序が時間で動くと「先に取り込みを頼んだ論文が先に終わる」という約束も崩れる。
  取り込みを頼み続けても後の論文が始まらない、という訴えが実際に出るまで入れない。

- **待ち時間を所要時間に足して見せる直し方をしない。**

  一覧の数字を、取り込みを頼んでから終わるまでの時間に統一する設計である。
  体感の待ち時間とは一致する。
...
これにより、先に取り込みを頼んだ論文のジョブが、どの段階でも後から取り込みを頼んだ論文の解決より先に走る。
...
解決のジョブは取り込みを頼まれた時点で積まれるので値を直接受け取り、以降の段階は取り込みの記録が持つ同じ時刻を読む。

## シナリオ

フィードの一覧を眺めて、20 本の論文の取り込みをまとめて頼む。

取り込みを頼んだ直後、進みの一覧には 20 行が並ぶ。
...
一覧の合計時間は 0 秒で、取り込みを頼んでから 10 分経っていることは行の見た目では分からない。
...
取り込みは画面から始めることも、会話のエージェントに頼んで始めることもある(0016)。
どちらの経路でも、取り込みを頼まれて解決のジョブが積まれた時刻がその取り込みの順序キーになる。
取得から登録までのジョブは、取り込みの記録が持つ「取り込みを頼まれた時刻」を読んで同じ値を使う。
取り込みの記録はこの時刻を既に持っている。
記録ができるのは解決が終わってからだが、そのときの現在時刻ではなく取り込みを頼まれた時刻を入れてあり、解決にかかった時間が記録から抜け落ちないようにしてある。

**やり直しでも順序キーを変えない。**
失敗した取り込みをやり直したとき、順序キーは最初に取り込みを頼まれた時刻のままである。
...
段階の間には順番待ちがあるので、この和は「取り込みを頼んでから終わるまで」より短くなる。
...
- **後から取り込みを頼んだ論文は、解決すら始まらないまま長く待つ。** 論文の取り込みを 20 本続けて頼むと、後半の論文は前半が読めるようになるまで題も分からない。進みの一覧には未解決の行として出るので、取り込みを頼んだことが消えるわけではない。
- **順序キーは飢餓を防がない。** 取り込みを頼み続ける限り、後の論文はいつまでも始まらない。取り込みを頼むのは人なので列はいつか空くが、自動で取り込む仕組みを将来入れるなら、この前提が崩れる。
- **失敗してやり直した取り込みは、頼んだ時刻のまま先頭に戻る。** 何日も放置した失敗をやり直すと、いま進んでいる論文を追い越す。やり直しを頼んだ本人はその論文を待っているので望ましい向きだが、他の論文の完了は遅れる。
- **一覧の合計時間は、体感の待ち時間より短く出る。** 実行時間だけを足すので、取り込みを頼んでから読めるまでの実時間とは一致しない。
