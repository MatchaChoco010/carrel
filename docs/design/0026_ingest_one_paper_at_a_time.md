# 取り込みを受け付けた順に 1 本ずつ流し、進みを実行時間で見せる

- created: 2026-08-03
- status: approved
- implementation: done (2026-08-03)

## 解決したい問題

論文をまとめて取り込んだとき、先に始めた 1 本が先に読めるようになる。
...
この既定が、段階の横並びを生む。
20 本の取り込みを始めると、Codex を資源クラスとするジョブの列に 20 件の解決が並ぶ。
...
- **待ち時間の長さで順序を動かさない。**
  ...
  後から受け付けた論文が延々と待たされる状態を防げる。
  しかし取り込みを起こすのは人であり、次々と取り込みを始めない限り列はいつか空く。
  順序が時間で動くと「先に受け付けた論文が先に終わる」という約束も崩れる。
  取り込みを次々と始めても後の論文が動かない、という訴えが実際に出るまで入れない。

- **待ち時間を所要時間に足して見せる直し方をしない。**

  一覧の数字を、取り込みを受け付けてから終わるまでの時間に統一する設計である。
  体感の待ち時間とは一致する。
...
ジョブに **順序キー** を持たせる。
これは「この仕事がどの取り込みのために積まれたか」を時刻で表す数値で、取り込みの鎖に属するジョブは、その取り込みを**受け付けた時刻**を全段階で共有する。
受け付けた時刻とは、画面の取り込むボタンが押されたか、会話のエージェントが取り込みの道具を呼んだかによって、その取り込みが carrel に届いた時刻である。
同じ資源クラスの中の実行順は、優先度、順序キー、積まれた順の 3 つで決まる。
これにより、先に受け付けた論文のジョブが、どの段階でも後から受け付けた論文の解決より先に走る。
...
```mermaid
flowchart LR
  T["取り込みを受け付けた時刻"] --> R["解決のジョブ"]
  T --> I["取り込みの記録"]
  I --> S["取得から登録までの各段階のジョブ"]
```

図の読み方: 矢印は、順序キーとして同じ値が渡る向きを表す。
解決のジョブは受け付けた時点で積まれるので値を直接受け取り、以降の段階は取り込みの記録が持つ同じ時刻を読む。

## シナリオ

フィードの一覧を眺めて、20 本の論文の取り込むボタンを続けて押す。

最後の 1 本を押した直後、進みの一覧には 20 行が並ぶ。
...
一覧の合計時間は 0 秒で、受け付けてから 10 分経っていることは行の見た目では分からない。
...
**取り込みの鎖に属するジョブの順序キーは、その取り込みを受け付けた時刻である。**
取り込みが始まる経路は 2 つある。
ユーザーが画面の取り込むボタンを押す経路と、ユーザーが会話のエージェントに取り込みを依頼し、エージェントが道具を呼ぶ経路である(0016)。
受け付けた時刻とは、どちらの経路でも、その取り込みが carrel に届いて解決のジョブが積まれた時刻を指す。
取得から登録までのジョブは、取り込みの記録が持つこの時刻を読んで同じ値を使う。
取り込みの記録はこの時刻を既に持っている。
記録ができるのは解決が終わってからだが、そのときの現在時刻ではなく受け付けた時刻を入れてあり、解決にかかった時間が記録から抜け落ちないようにしてある。

**やり直しでも順序キーを変えない。**
失敗した取り込みをやり直したとき、順序キーは最初に受け付けた時刻のままである。
...
**待機のまま取り残される段階は無い。**
段階を積むというのはジョブをキューに入れることであり、キューは `state.sqlite` に永続化してサーバーの再起動をまたいで復元する(0003)。
待機のジョブは再起動後もそのまま残り、枠が空いた順に走り出す。
落ちた時点で走っていたジョブは待機へ戻され、同じように積み直される。
待機の段階が動かなくなるのは、残枠の枯渇のようにキュー自体が止まっているときだけで、これは再起動とは別の話である。
...
段階の間には順番待ちがあるので、この和は「受け付けてから終わるまで」より短くなる。
...
- **後から受け付けた論文は、解決すら始まらないまま長く待つ。** 論文の取り込みを 20 本続けて始めると、後半の論文は前半が読めるようになるまで題も分からない。進みの一覧には未解決の行として出るので、受け付けたことが消えるわけではない。
- **順序キーは飢餓を防がない。** 取り込みを次々と始める限り、後の論文はいつまでも動かない。取り込みを起こすのは人なので列はいつか空くが、自動で取り込む仕組みを将来入れるなら、この前提が崩れる。
- **失敗してやり直した取り込みは、最初に受け付けた時刻のまま先頭に戻る。** 何日も放置した失敗をやり直すと、いま進んでいる論文を追い越す。やり直しを始めた本人はその論文を待っているので望ましい向きだが、他の論文の完了は遅れる。
- **一覧の合計時間は、体感の待ち時間より短く出る。** 実行時間だけを足すので、受け付けてから読めるまでの実時間とは一致しない。
- **中断した段階の所要時間は、走り直したぶんだけになる。** 走っている途中でサーバーが落ちると、その段階は待機へ戻って最初から走り直す。走り出した時刻も上書きされるので、落ちる前に使った時間は数字に残らない。
