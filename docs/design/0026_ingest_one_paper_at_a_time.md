...
- **待ち時間の長さで順序を動かさない。**
  ...
  しかし取り込みを始めるのは人であり、頼み続けない限り列はいつか空く。
...
一覧の合計時間は 0 秒で、頼んでから 10 分経っていることは行の見た目では分からない。
...
どちらの経路でも、頼まれて解決のジョブが積まれた時刻がその取り込みの順序キーになる。
取得から登録までのジョブは、取り込みの記録が持つ「頼まれた時刻」を読んで同じ値を使う。
取り込みの記録はこの時刻を既に持っている。
記録ができるのは解決が終わってからだが、そのときの現在時刻ではなく頼まれた時刻を入れてあり、解決にかかった時間が記録から抜け落ちないようにしてある。

**やり直しでも順序キーを変えない。**
失敗した取り込みをやり直したとき、順序キーは最初に頼まれた時刻のままである。
...
- **後から頼んだ論文は、解決すら始まらないまま長く待つ。** 20 本を続けて頼むと、後半の論文は前半が読めるようになるまで題も分からない。進みの一覧には未解決の行として出るので、頼んだことが消えるわけではない。
- **順序キーは飢餓を防がない。** 取り込みを頼み続ける限り、後の論文はいつまでも始まらない。頼むのは人なので列はいつか空くが、自動で取り込む仕組みを将来入れるなら、この前提が崩れる。
- **失敗してやり直した取り込みは、頼んだ時刻のまま先頭に戻る。** 何日も放置した失敗をやり直すと、いま進んでいる論文を追い越す。やり直しを頼んだ本人はその論文を待っているので望ましい向きだが、他の論文の完了は遅れる。
- **一覧の合計時間は、体感の待ち時間より短く出る。** 実行時間だけを足すので、頼んでから読めるまでの実時間とは一致しない。
