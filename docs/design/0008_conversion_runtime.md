...
- created: 2026-07-28
- status: approved
- implementation: done (2026-07-29)
...
**ROCm のバージョンによって、視覚モデルの出力が壊れることがある。**
...
つまり、変換の正しさは ROCm のどのバージョンを使うかに依存する。
...
- **ROCm を自動更新に任せない。** バージョンは固定し、更新は人が判断して行う。変換の正しさが特定のバージョンの数値計算に依存することは上に述べたとおりで、自動更新に任せると、ある日から論文が静かに壊れて取り込まれる。
- **文字を失う取り出し経路の問題を、変換器側で直さない。** この欠落は変換器 2 つに共通して存在し、共有している下位の実装に由来する。上流の修正を待つことも、こちらで手を入れたバージョンを持つこともしない。手を入れたバージョンを抱えると、そのバージョンの保守が変換器の更新のたびに発生する。対処は照合の段階で行う(0009)。
...
GPU を駆動する ROCm は、安定版を使い、バージョンを固定する。
変換の正しさがバージョンに依存する以上、どのバージョンで動かしているかが決まっていなければ、出力が正しいことを主張できない。
...
時間の測定値をバージョンの比較に使うときは、1 回の測定で判断してはならない。
...
carrel が依存するのはこの形までとし、変換器が出力する markdown の書き方には依存しない。
...
- **ROCm のバージョンを固定するため、更新は人の作業になる。** バージョンを上げるときは同じ論文で出力が変わらないことを確かめる必要があり、その手間が更新のたびに発生する。確かめずに上げると、崩壊に気づかないまま論文が取り込まれる。
...
- **変換を CPU で行う。**
  ...
  GPU を使わない構成で、環境の構築が単純になり、ROCm のバージョンに依存しなくなる。
...
- **ROCm の開発版を使う。**
  ...
  gfx1200 に的を絞ってビルドされたバージョンで、公式の安定版より新しい。
...
carrel は個人が自分の論文を読むために使うもので、第三者へサービスを提供しない(0007)。
