...
test('Wayland と Vulkan の指定を必ず渡す', () => {
  const script = wrapperScript('pct-bin')
...
test('渡された引数をそのまま引き継ぐ', () => {
  assert.match(wrapperScript('pct-bin'), /"\$@"$/m)
})
...
test('自分の置き場所から実行ファイルを引く', () => {
  ...
  assert.match(script, /exec "\$directory\/pct-bin"/)
})

test('シェルの指定で始まる', () => {
  assert.ok(wrapperScript('pct-bin').startsWith('#!/bin/sh\n'))
})
