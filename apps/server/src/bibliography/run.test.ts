...
test('プレプリントの投稿先は学会名として採らない', () => {
  const found = parseBibliography(
    JSON.stringify({ venue: 'arXiv preprint', year: 2026, doi: null, arxivId: null, pdfUrl: null, pageUrl: null }),
  )
...
test('DOI として読めない値は落とす', () => {
  const found = parseBibliography(
    JSON.stringify({ venue: null, year: null, doi: '見つからなかった', arxivId: null, pdfUrl: null, pageUrl: null }),
  )
...
test('URL でない所在は採らない', () => {
  const found = parseBibliography(
    JSON.stringify({ venue: null, year: null, doi: null, arxivId: null, pdfUrl: 'ACM のページ', pageUrl: null }),
  )
