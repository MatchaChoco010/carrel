...
test('ページごとの変更点と理由を読める', () => {
  const md = buildReport([{ page: 3, changes: [change()], remaining: null }])
...
test('決めがたかった箇所も残す', () => {
  const md = buildReport([{ page: 0, changes: [change({ kind: 'undecided', source: 'none' })], remaining: null }])
  assert.match(md, /未決/)
...
test('変更も欠落も無いページは載せない', () => {
  const md = buildReport([
    { page: 0, changes: [], remaining: null },
    { page: 1, changes: [change()], remaining: null },
  ])
...
test('表を壊す文字を含む変更点でも表の形を保つ', () => {
  const md = buildReport([
    { page: 0, changes: [change({ before: 'a | b', after: '複数\n行' })], remaining: null },
  ])
...
test('全体の件数を冒頭に書く', () => {
  const md = buildReport([
    { page: 0, changes: [change(), change()], remaining: null },
    { page: 1, changes: [], remaining: null },
  ])
