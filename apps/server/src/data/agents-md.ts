...
応答の中で論文の図を出すときは、次の形で書く。

\`\`\`md
![図 1 の説明](@<slug>/assets/<画像のファイル名>)
\`\`\`

画像のファイル名は \`papers/<slug>/assets/\` にあるものである。
この形で書いた図だけが、ユーザーの画面で図として出る。
ファイルの場所をそのまま書いたり、外の URL を書いたりしても表示されない。
