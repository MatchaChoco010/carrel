 ...
 * ページを繋いだ後に直す。
 *
 * 判定は機械的に行う。前の段落が文の終わりに見えず、次の段落が続きに見えるときだけ
 * 繋ぐ。エージェントに諮る必要はない。
 */
...
/** 文の終わりに見える末尾。ここで終わっていれば次の段落は別の文である。 */
const SENTENCE_END = /[.!?:;)\]}"'’”%]$/

/** 続きに見える先頭。小文字で始まるか、括弧で始まる。 */
const CONTINUES = /^[a-z(\[]/

/** 行末で割れた語。ハイフンは繋ぐときに落とす。 */
const BROKEN_WORD = /[-‐‑]$/
 ...
 * 2 つの段落を繋ぐかを決める。
 *
 * 末尾のハイフンには 2 通りの意味がある。次が小文字なら行末で割れた語なので
 * ハイフンを落とし、大文字なら `Fridovich-Keil` のような元からのハイフンなので残す。
 */
