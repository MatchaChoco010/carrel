...
- **ブランチを消す前に、その PR が本当にマージされたかを 1 本ずつ確かめる。** GitHub は**ブランチが消えると、そこを宛先にしている PR を自動で閉じる**。まだマージされていない PR のブランチを消すとその PR 自身が閉じ、積んだ PR の土台を消すと上の PR が閉じる。閉じた PR は宛先を変えられず、`pr reopen` も宛先のブランチが無いと失敗するので、戻すにはブランチを作り直すことになる(下記「巻き添えで閉じた PR を戻す」)。
- **PR を積んだときは、土台をマージする前に上の PR の宛先を develop へ付け替える**(下記「PR を積むとき」)。
- `git fetch` / `git pull` でローカルをリモートに追従させ続ける。

## PR を積むとき

実装を分けて、手前の PR がまだマージされていないうちに続きを出すことがある。
続きの差分だけをレビューできるように、**上の PR の宛先を手前の PR のブランチにする**(積む)。

畳むときの順番を守る。

1. 上の PR の**宛先を develop へ付け替える**(`gh pr edit <上の PR> --base develop`)。
2. 手前の PR をマージし、そのブランチを消す。
3. 上の PR のコンフリクトを解消してからマージする。

順番を逆にして手前のブランチを先に消すと、上の PR が閉じる。
`--delete-branch` は「消す前に確かめる」の機会を飛ばすので、積んだ PR があるうちは使わない。

## 巻き添えで閉じた PR を戻す

ブランチを消したせいで PR が閉じたときは、次の順で戻す。
閉じた PR は宛先を変えられないので、**先にブランチを作り直す**。

1. 消したブランチを、マージ済みのコミット(または元の先端の SHA)から push し直す。
   ```sh
   git push origin <SHA>:refs/heads/<消したブランチ>
   ```
2. `gh pr reopen <PR番号>` で開き直す。
3. `gh pr edit <PR番号> --base develop` で宛先を付け替える。
4. 作り直したブランチを消す。
5. develop を取り込んでコンフリクトを解消する。

PR を作り直すのではなく開き直すのは、レビューのやりとりと承認を失わないためである。
